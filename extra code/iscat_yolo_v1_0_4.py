...
# Particle Class
class Particle:
    def __init__(self, px, py, wx, wy, first_frame_seen, pID, yoloimage, pimage, stddev, mean, conf):
...
def ratio_particle_finder(images, constants):
    ...
    for f in tqdm.tqdm(range(n-2*bufsize)):
        ...
        ''' FIND IF PARTICLE EXISTED IN A PAST FRAME  '''
        for p in frame_pl:
            ...
            new_particle = Particle(px, py, wx, wy, f, pID, yoloimage, pimage, stddev, mean, conf)
